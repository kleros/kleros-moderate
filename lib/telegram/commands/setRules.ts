...
const callback = async (queue: any, db: any, settings: groupSettings, bot: any, botId: string, msg: any, match: string[]) => {
    try{
        if (msg.reply_to_message && !msg.reply_to_message.forum_topic_created) {
            ...
            queue.add(async () => {try{await bot.sendMessage(msg.chat.id, `The community rules are [updated](https://ipfs.kleros.io${rulesPath}).`, msg.chat.is_forum? {message_thread_id: settings.thread_id_rules, parse_mode: 'Markdown'}: {parse_mode: "Markdown"})}catch{}});
            ...
            queue.add(async () => {try{await bot.sendMessage(msg.chat.id, langJson[settings.lang].errorRules, msg.chat.is_forum? {message_thread_id: settings.thread_id_rules}: {})}catch{}});
        }
    } catch(e){
        console.log(e)
    }
