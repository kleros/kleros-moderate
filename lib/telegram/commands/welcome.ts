...
const callback = async (queue: any, settings: groupSettings, bot: any, msg: any) => {
    try {
        if (msg.old_chat_member.status !== "left")
            return;
        queue.add(async () => {try{await bot.sendMessage(msg.chat.id, `${langJson[settings.lang].welcome}[Kleros Moderate](https://kleros.io/moderate/) bot. \n\n/start to begin community sourced moderation.`, msg.chat.is_forum? {parse_mode: "Markdown"} : {message_thread_id: msg.message_thread_id, parse_mode: "Markdown"})}catch{}});
