...
(async () => {
     ...
     */
     await db.exec('CREATE TABLE accounts (address TEXT PRIMARY KEY, private_key TEXT, platform TEXT, user_id TEXT)');
     ...
     */
    await db.exec('CREATE TABLE groups (platform TEXT, group_id TEXT, invite_url TEXT, address TEXT, PRIMARY KEY (platform, group_id))');
     ...
     */
    await db.exec('CREATE TABLE reports (question_id TEXT PRIMARY KEY, platform TEXT, group_id TEXT, user_id TEXT, msg_id TEXT, timestamp INTEGER, activeTimestamp INTEGER, active BOOLEAN, timeServed INTEGER, finalized BOOLEAN, evidenceAllowance INTEGER)');
    /**
     * `platform` can be `telegram`, `reddit`, etc.
     * `group_id` is the id of the group on the platform (eg. group on telegram, subreddit on reddit, etc.).
     ...
     */
    await db.exec('CREATE TABLE rules (platform TEXT, group_id TEXT, rules TEXT, timestamp INTEGER, PRIMARY KEY (platform, group_id))');
